...
def merge_stocks(individual_folder="individual_stocks", processed_folder="processed_stocks"):
    ...
    for i, f in enumerate(stock_files):
        try:
            ...
            # Attempt to detect the column names
...
@st.cache_resource
def train_lstm_model(series, epochs=10, seq_length=30):
...
# -----------------------------
@st.cache_resource
def train_tft_model(df, max_encoder=30, max_pred=30, epochs=5):
...
def main():
    ...
    if tft_available:
        ...
        st.info("ℹ️ TFT skipped: series too short or invalid")
    ...
    colors = {
        'Actual':'white','ARIMA':'orange','Prophet':'green',
        'LSTM Mean':'cyan','LSTM CI':'lightcyan','TFT Mean':'magenta','TFT CI':'pink'
    }

    # Actual
    fig.add_trace(go.Scatter(x=df['date'], y=df['value'], mode='lines', name='Actual', line=dict(color=colors['Actual'], dash='dash')))
    # ARIMA
